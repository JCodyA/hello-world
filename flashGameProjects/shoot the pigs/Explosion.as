package  {
	...
	public class Explosion extends MovieClip{
		...
		private static const explodeLimit:uint = 100;
		private var mute:Boolean = false;
		private var isPaused:Boolean = false;
		private var xScaler:int = 15;
		private var yScaler:int = 15;
		...
		public function Explosion(stageRef:Stage) {
			this.addEventListener(Event.ENTER_FRAME, eframe);
			tween1 = new Tween(this,"width",Strong.easeInOut,1,Explosion.explodeLimit,.25,true);
			tween2 = new Tween(this,"height",Strong.easeInOut,1,Explosion.explodeLimit,.25,true);
			...
			greyData1.perlinNoise(25,25,5,Math.random()*512+1,true,false,0,true,offsetArray);
			mapContainer1.addChild(greyMap1);
			greyData2.perlinNoise(25,25,5,Math.random()*512+1,true,false,0,true,offsetArray);
		...
		private function eframe (e:Event):void {
			if (isPaused == false) {
				...
				greyData3.applyFilter(greyData3,greyData3.rect,zeroPoint,convolution);
				finalData.paletteMap(greyData3,greyData3.rect,zeroPoint,colors,zeroArray,zeroArray,zeroArray);
				if (this.width == Explosion.explodeLimit && this.isFading == false) {
					tween3 = new Tween(this,"alpha",None.easeOut,1,0,.25,true);
					this.isFading = true;
				}
				if (this.alpha == 0) {
					removeMe();
